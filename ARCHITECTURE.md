...
- Fase 19.3 planejada para polimento visual, consistencia de marca e UX comercial; Fase 19.4 planejada para performance e regressao apos congelamento visual; Fase 20 planejada para preparacao final de producao.
- HTMX quando houver interacao real que justifique sua presenca.

## Evolucao planejada pre-go-live

A Fase 17.1 foi concluida e validada em producao: slugs sao gerados no servidor para cadastros administrativos, preservados em renomeacoes e recebem sufixos determinísticos em colisoes. A Fase 17.2 foi concluida e validada em producao: `products.shipping_*` e a fonte autoritativa, configuracoes nao alteram frete e caixas usam um conjunto dimensional operacional no Admin. A Fase 17.3 foi concluida e validada em producao: cores comerciais ficam separadas da receita, podem ser escolhidas na pagina publica e persistem em `cart_items.color_id`, com unicidade por produto/variante/cor e snapshot de ID/nome/slug em `order_items`, exibido no Admin. A selecao e revalidada pelo backend e nao altera receita, frete ou pagamento. Retirada no local tambem foi concluida e validada em producao, com `delivery_method=pickup`, frete zero server-side e sem SuperFrete/caixa/perfil logistico. A Fase 18 adicionou a fundacao de e-mails transacionais, autenticacao publica de clientes com Supabase Auth, Minha Conta, perfil persistente, preenchimento automatico do checkout e retomada autenticada de pagamentos pendentes; a Fase 18.4 foi concluida e validada em producao mantendo checkout convidado. O Admin continua separado por senha, TOTP, AAL2, allowlist e sessao propria. O SMTP externo para autenticacao foi configurado fora do repositorio; credenciais permanecem somente no provedor. A Fase 19.3 planeja polimento visual, consistencia de marca e UX comercial antes da Fase 19.4 de performance/regressao; a Fase 20 fara a auditoria final de producao somente apos essas etapas estabilizarem.
...
A etapa de dados do checkout continua sem exigir login. Contato e endereco pertencem ao carrinho anonimo atual e nao sao automaticamente vinculados a conta de cliente nesta fase. Esses dados sao PII e devem ser tratados com minimizacao, validacao server-side, leitura consistente, `Cache-Control: private, no-store` em respostas HTML que possam conter PII e erros genericos. A consulta de CEP e uma melhoria progressiva feita pelo backend contra ViaCEP; o navegador nao chama ViaCEP diretamente e o preenchimento manual continua valido.
...
A integracao SuperFrete usa `net/http`, timeout explicito, `Authorization: Bearer <token>` e `User-Agent` operacional. O backend mapeia internamente `sandbox` para `https://sandbox.superfrete.com` e `production` para `https://api.superfrete.com`; nao ha base URL arbitraria por environment variable. A PrintLab escolhe primeiro a menor caixa fisica real que comporta os itens com empacotamento 3D conservador, soma o peso dos produtos e da embalagem, e envia somente `package` com dimensoes externas e peso final para obter o preco apresentado ao cliente. Falhas de cotacao sao classificadas internamente por estagio e motivo seguro, sem registrar CEP, CPF, e-mail, telefone, endereco, token ou corpo bruto externo.
...
GET /cadastro -> formulario SSR de cadastro de cliente
POST /cadastro -> cria usuario no Supabase Auth e exige confirmacao de e-mail
GET /login -> formulario SSR de login de cliente
POST /login -> autentica cliente no Supabase Auth e grava cookies HttpOnly com a sessao Supabase
POST /logout -> logout de cliente no Supabase Auth e limpeza dos cookies de cliente
GET /recuperar-senha -> formulario SSR de recuperacao
POST /recuperar-senha -> solicita recuperacao oficial ao Supabase Auth com mensagem sem enumeracao
GET /recuperar-senha/nova -> formulario protegido para nova senha apos callback
POST /recuperar-senha/nova -> atualiza senha no Supabase Auth e encerra sessao atual
GET /auth/callback -> pagina noindex que conclui callback de confirmacao/recuperacao
POST /auth/session -> recebe tokens do callback Supabase e grava cookies HttpOnly
GET /conta -> area inicial protegida da conta de cliente
