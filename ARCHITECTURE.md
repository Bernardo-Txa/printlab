...
O fluxo publico de cliente usa Supabase Auth como fonte oficial de identidade e sessao. Rotas `/cadastro`, `/login`, `/recuperar-senha`, `/auth/callback`, `/auth/session`, `/conta` e `POST /logout` ficam separadas do Admin. O backend nao cria tabela de cliente, nao armazena senha, nao cria hash proprio, nao cria token de recuperacao proprio e nao persiste access token/refresh token no banco. Para SSR, os tokens emitidos pelo Supabase ficam em cookies HttpOnly `printlab_customer_access_token` e `printlab_customer_refresh_token`, SameSite=Lax, Path=/ e Secure em producao/HTTPS. O servidor valida usuario em `GET /auth/v1/user`, renova por `grant_type=refresh_token` quando necessario e limpa cookies no logout.
