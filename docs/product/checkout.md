...
Se nao houver carrinho valido, a rota redireciona para `/carrinho`. Se os dados ainda nao existirem, redireciona para `/checkout/dados`. Produto sem perfil logistico, ausencia de caixa ativa ou configuracao SuperFrete ausente retorna estado operacional de indisponibilidade sem inventar preco.

O backend calcula frete com a PrintLab como fonte de verdade da embalagem fisica: expande os itens por quantidade, testa rotacoes e posicoes sem sobreposicao nas caixas ativas e escolhe a menor caixa real compativel. Quando nenhuma caixa real comporta os itens, usa uma embalagem estimada conservadora sem gravar `shipping_box_id`. Em ambos os casos, faz uma unica cotacao SuperFrete com `package` usando medidas e peso finais. A SuperFrete define preco, prazo e servicos disponiveis; ela nao escolhe a caixa da PrintLab.
