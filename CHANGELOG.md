...
- Planejamento da Fase 19.3 substituido por polimento visual, consistencia de marca e UX comercial; Fase 19.4 registrada para performance e regressao de producao apos congelamento visual.

- Fase 18.4 concluida e validada em producao: retomada autenticada de pagamento pendente em `/conta`, com `POST /conta/pedidos/{id}/pagar`, autorizacao por `orders.customer_auth_user_id`, reuso de `internal/payments`, checkout convidado preservado e sem nova migration.
...
- Fase 14.3: plano operacional para rate limiting no Vercel Firewall por IP, com quatro POSTs sensiveis, webhook InfinitePay excluido e Bot Protection em Log. A configuracao e validacao real no projeto Vercel permanecem pendentes de autenticacao.
...
- Fase 10 — Pagamentos InfinitePay, com checkout hospedado server-side, retorno por `payment_check` e validacao real concluida.
...
- Canonicalizacao de host para `GET` e `HEAD` baseada em `SITE_URL`, preservando validacao `Origin`/`Referer` em POST.
