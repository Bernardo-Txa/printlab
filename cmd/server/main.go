...
func newHandlerWithServicesAndOrdersAndCustomerAuthAndSupabaseURL(db *database.Database, catalog catalogService, shoppingCart cartService, checkoutDetails checkoutDetailsService, checkoutShipping checkoutShippingService, orderReview orderReviewService, cartCookies *cartdomain.CookieManager, postalCodeLookup postalCodeLookupService, payment paymentService, adminPanel adminPanelService, customerAuth customerAuthService, siteURL string, supabaseURL string) http.Handler {
	...
	mux.HandleFunc("GET /conta", accountHandler(customerAuth, orderReview, profileService, payment, siteURL))
	mux.HandleFunc("POST /conta", accountSaveHandler(customerAuth, orderReview, profileService, siteURL))
	mux.HandleFunc("POST /conta/pedidos/{id}/pagar", accountResumePaymentHandler(customerAuth, payment, siteURL))
	mux.HandleFunc("GET /conta/pedidos/{id}/pagar", methodNotAllowedHandler(http.MethodPost))
	...
	mux.HandleFunc("GET /checkout/dados", checkoutDetailsPageHandler(checkoutDetails, cartCookies, profileService, orderReview))
	...
	mux.HandleFunc("GET /checkout/frete", checkoutShippingPageHandler(checkoutShipping, cartCookies, profileService))
	mux.HandleFunc("POST /checkout/frete/endereco", saveShippingAddressHandler(checkoutShipping, cartCookies, siteURL, profileService))
	mux.HandleFunc("GET /checkout/frete/endereco", methodNotAllowedHandler(http.MethodPost))
